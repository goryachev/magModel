...
#ifndef HELPER_MATH_WO_CUDA_H
#define HELPER_MATH_WO_CUDA_H
...
inline float2 make_float2(float x, float y) { float2 res; res.x=x; res.y=y; return res; }
inline float3 make_float3(float x, float y, float z) { float3 res; res.x=x; res.y=y; res.z=z; return res; }
inline float4 make_float4(float x, float y, float z, float w) { float4 res; res.x=x; res.y=y; res.z=z; res.w=w; return res; }

inline short2 make_short2(short x, short y) { short2 res; res.x=x; res.y=y; return res; }

inline int2 make_int2(int x, int y) { int2 res; res.x=x; res.y=y; return res; }
inline int3 make_int3(int x, int y, int z) { int3 res; res.x=x; res.y=y; res.z=z; return res; }
inline int4 make_int4(int x, int y, int z, int w) { int4 res; res.x=x; res.y=y; res.z=z; res.w=w; return res; }

inline uint2 make_uint2(unsigned int x, unsigned int y) { uint2 res; res.x=x; res.y=y; return res; }
inline uint3 make_uint3(unsigned int x, unsigned int y, unsigned int z) { uint3 res; res.x=x; res.y=y; res.z=z; return res; }
inline uint4 make_uint4(unsigned int x, unsigned int y, unsigned int z, unsigned int w) { uint4 res; res.x=x; res.y=y; res.z=z; res.w=w; return res; }
inline uchar4 make_uchar4(unsigned char x, unsigned char y, unsigned char z, unsigned char w) { uchar4 res; res.x=x; res.y=y; res.z=z; res.w=w; return res; }
...
inline float2 swap(float2 s) { return make_float2(s.y, s.x); }
...
inline int3 make_int3(dim3 a) { return make_int3(int(a.x), int(a.y), int(a.z)); }
...
inline uint3 make_uint3(dim3 a) { return make_uint3(uint(a.x), uint(a.y), uint(a.z)); }
...
// subtract
////////////////////////////////////////////////////////////////////////////////
inline uchar4 operator-(uchar4 &a, uchar4 &b) { return make_uchar4(a.x-b.x, a.y-b.y, a.z-b.z, a.w-b.w); }
...
inline int2 abs(int2 v) { return make_int2(iabs(v.x), iabs(v.y)); }
inline int3 abs(int3 v) { return make_int3(iabs(v.x), iabs(v.y), iabs(v.z)); }
inline int4 abs(int4 v) { return make_int4(iabs(v.x), iabs(v.y), iabs(v.z), iabs(v.w)); }
...
inline float smoothstep(float a, float b, float x) { float y = clamp((x - a) / (b - a), 0.0f, 1.0f); return (y*y*(3.0f - (2.0f*y))); }
inline float2 smoothstep(float2 a, float2 b, float2 x) { float2 y = clamp((x - a) / (b - a), 0.0f, 1.0f); return (y*y*(make_float2(3.0f) - (make_float2(2.0f)*y))); }
inline float3 smoothstep(float3 a, float3 b, float3 x) { float3 y = clamp((x - a) / (b - a), 0.0f, 1.0f); return (y*y*(make_float3(3.0f) - (make_float3(2.0f)*y))); }
inline float4 smoothstep(float4 a, float4 b, float4 x) { float4 y = clamp((x - a) / (b - a), 0.0f, 1.0f); return (y*y*(make_float4(3.0f) - (make_float4(2.0f)*y))); }

//struct double2 { double x,y; };

//inline double2 make_double2(double x, double y) { double2 res; res.x=x; res.y=y; return res; }
#endif

inline double2 make_double2(double x) { return make_double2(x,x); }
inline void operator+=(double2 &a, float2 b) { a.x += b.x; a.y += b.y; }
